...
HtmlTextStyle _style(String css) {
  final document =
...
void main() {
  ...
  test('falls back to Helvetica for an unknown family', () {
    expect(
        HtmlStandardFont.resolve(_style('font-family: Papyrus;')), 'Helvetica');
    expect(
        HtmlStandardFont.resolve(_style('font-family: serif;')), 'Times-Roman');
    expect(
        HtmlStandardFont.resolve(
  ...
  test('normal CSS weight and style override inherited semantic emphasis', () {
    ...
    expect(HtmlStandardFont.resolve(leaf), 'Courier');
  });
}
