...
void main() {
  group('PdfPercentCommentLine', () {
    test('toAsciiSafe', () {
      final line = PdfPercentCommentLine(
          offset: 0, bytes: Uint8List.fromList([0x25, 0x41, 0x01, 0x42]));
      expect(line.toAsciiSafe(), equals('%A.B'));
    });

    test('toHex', () {
      final line = PdfPercentCommentLine(
          offset: 0, bytes: Uint8List.fromList([0x25, 0xAB]));
  ...
  group('extractPdfPercentCommentLines', () {
    ...
    test('ignores % in middle if startOfLineOnly is true', () {
      ...
      final comments =
          extractPdfPercentCommentLines(bytes, startOfLineOnly: true);
    ...
    test('finds % in middle if startOfLineOnly is false', () {
      ...
      final comments =
          extractPdfPercentCommentLines(bytes, startOfLineOnly: false);
