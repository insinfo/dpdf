...
class ASN1Utils {
  ...
  static Uint8List encodeTagged(int tag, Uint8List value) {
  ...
  static Uint8List createSequence(List<Uint8List> elements) {
    ...
    return encodeTagged(tagSequence, builder.toBytes());
  }
  ...
  static Uint8List createSet(List<Uint8List> elements) {
    ...
    return encodeTagged(tagSet, builder.toBytes());
  }
  ...
  static Uint8List createInteger(BigInt value) {
    ...
    return encodeTagged(tagInteger, bytes);
  }
  ...
  static Uint8List createOctetString(Uint8List data) {
    return encodeTagged(tagOctetString, data);
  }
  ...
  static Uint8List createBitString(Uint8List data, {int unusedBits = 0}) {
    ...
    builder.add(data);
    return encodeTagged(tagBitString, builder.toBytes());
  }
  ...
  static Uint8List createOID(String oid) {
    ...
    return encodeTagged(tagOid, builder.toBytes());
  }
  ...
  static Uint8List createBoolean(bool value) {
    return encodeTagged(tagBoolean, Uint8List.fromList([value ? 0xFF : 0x00]));
  }

  /// Creates an ASN.1 UTF8 STRING.
  static Uint8List createUtf8String(String value) {
    return encodeTagged(tagUtf8String, Uint8List.fromList(value.codeUnits));
  }
  ...
  static Uint8List createUtcTime(DateTime dateTime) {
    ...
    return encodeTagged(tagUtcTime, Uint8List.fromList(formatted.codeUnits));
  }
  ...
  static Uint8List createTagged(int tagNumber, Uint8List content,
    ...
    if (isExplicit) {
      return encodeTagged(tag, content);
    } else {
  ...
  /// Parses an OID from ASN.1 content bytes.
  static String parseOID(Uint8List content) {
    ...
    int value = 0;
    // Note: This simple implementation might overflow for very large arcs (BigInt needed for generic)
    // But for standard OIDs it usually fits in int (64-bit in Dart).
    // Let's use BigInt if needed, but int is usually fine for OIDs we care about.
    // Actually, some OID components can be large.

    // Using BigInt logic for safety?
    // Let's stick to int for now as in PdfPKCS7.
...
class ASN1ParseResult {
  ...
  /// The full encoded bytes (Tag + Length + Content).
  final Uint8List? _fullEncoded;
  ...
  Uint8List get encodedBytes {
    if (_fullEncoded != null) return _fullEncoded;
    // Fallback: reconstruct (might not match original exactly if indefinite length used, etc)
    return ASN1Utils.encodeTagged(tag, content);
  }
  ...
  bool get isContextSpecific =>
      (tag & 0xC0) == 0x80 ||
      (tag & 0xC0) == 0xA0; // Context specific can be constructed (A0)
