...
void _expectCoordinates(SvgPathSegment segment, List<double> expected) {
...
void main() {
  group('SvgPathParser', () {
    test('normaliza comandos absolutos em mover, reta e fechar', () {
      final segments = SvgPathParser.parse('M10 20 L30 40 Z');
    ...
    test('acumula deslocamentos relativos e repete o comando implícito', () {
      ...
      final segments = SvgPathParser.parse('m10 10 5 5 5 5');
    ...
    test('resolve retas horizontais e verticais mantendo o outro eixo', () {
      final segments = SvgPathParser.parse('M0 0 H10 v5 h-4');
    ...
    test('reflete o controle da cúbica anterior em S', () {
      final segments = SvgPathParser.parse('M0 0 C1 1 2 2 3 3 S5 5 6 6');
    ...
    test('lê os sinalizadores do arco colados ao número seguinte', () {
      ...
      final segments = SvgPathParser.parse('M0 0 a5 5 0 011 1');

      expect(segments.first.op, SvgPathOp.moveTo);
      expect(segments.skip(1).every((s) => s.op == SvgPathOp.curveTo), isTrue);
    ...
    test('arco de raio nulo vira reta até o destino', () {
      final segments = SvgPathParser.parse('M0 0 A0 0 0 0 1 10 10');

      expect(segments[1].op, SvgPathOp.lineTo);
      _expectCoordinates(segments[1], [10, 10]);
    });

    test('meia volta produz duas cúbicas terminando no ponto pedido', () {
      final segments = SvgPathParser.parse('M0 0 A5 5 0 0 1 10 0');
    ...
    test('fechar o subcaminho devolve o ponto corrente ao início dele', () {
      final segments = SvgPathParser.parse('M10 10 L20 20 Z l5 5');

      expect(segments.last.op, SvgPathOp.lineTo);
      _expectCoordinates(segments.last, [15, 15]);
    });

    test('aplica a escala de unidade na emissão', () {
      final segments = SvgPathParser.parse('M10 20 L30 40', unitScale: 0.75);
    ...
    test('preserva o prefixo válido de um caminho truncado', () {
      final segments = SvgPathParser.parse('M10 10 L20');

      expect(segments.length, 1);
      expect(segments.single.op, SvgPathOp.moveTo);
    });

    test('ignora um caminho que começa sem comando', () {
      expect(SvgPathParser.parse('10 10 L20 20'), isEmpty);
      expect(SvgPathParser.parse('   '), isEmpty);
      expect(SvgPathParser.parse(null), isEmpty);
    });

    test('separa números colados por ponto decimal e por sinal', () {
      final segments = SvgPathParser.parse('M1.5.5L-1-2');
