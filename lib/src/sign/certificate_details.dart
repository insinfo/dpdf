...
///
/// Exposes certificate fields independently of their storage or parser.
abstract class CertificateDetails {
  /// Gets the issuer distinguished name.
  ///
  /// @return the issuer DN as a string
  String getIssuerDN();

  /// Gets the issuer X.500 name as encoded bytes.
  ...
  /// Gets the Subject Key Identifier extension value.
  ///
  /// @return the raw subject key identifier bytes, or null if not present
  Uint8List? getSubjectKeyIdentifier();
}

/// Factory for creating X.509 certificates.
abstract class CertificateDecoder {
  /// Parses a certificate from DER-encoded bytes.
  ///
  /// @param encoded the DER-encoded certificate
  /// @return the parsed certificate
  CertificateDetails parseCertificate(Uint8List encoded);
  ...
  List<CertificateDetails> parseCertificates(Uint8List encoded);
}
