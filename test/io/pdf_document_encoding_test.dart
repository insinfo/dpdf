...
void main() {
  const encoding = PdfEncodings.PDF_DOC_ENCODING;
  String decode(List<int> bytes) =>
      PdfEncodings.convertToString(Uint8List.fromList(bytes), encoding);
  Uint8List encode(String text) => PdfEncodings.convertToBytes(text, encoding);
  ...
  test('PDF document character assignments differ from Windows ANSI', () {
    ...
    expect(
        PdfEncodings.convertToString(
            Uint8List.fromList([0x80]), PdfEncodings.WINANSI),
        '\u20ac');
  });
  ...
  test('ASCII, Latin letters and valid whitespace are preserved', () {
    ...
    expect(PdfEncodings.convertToString(encode(text), 'pdf'), text);
  ...
  test('UTF encodings and raw bytes remain independent of PDF encoding', () {
    ...
    for (final encoding in [
      ...
      expect(
          PdfEncodings.convertToString(
              PdfEncodings.convertToBytes(text, encoding), encoding),
          text);
    }
    expect(PdfEncodings.convertToString(Uint8List.fromList([0x80, 0xad]), null),
        '\u0080\u00ad');
  });
}
